...
let x0Coord = coordinate.0
let y1Coord = coordinate.1
print(x0Coord, y1Coord)
/*:
...
let a = startCoord
let b = endCoord
let trip = sqrt((b.x - a.x)*(b.x - a.x) + (b.y - a.y)*(b.y - a.y))
print(trip)

var usdMoney : (amount: Double, type:String) = (1.00, "usd")
var cndMoney : (amount: Double, type:String) = (1*1.29, "cnd")
print("usValue = \(usdMoney.amount), usType = \(usdMoney.type) is cndValue = \(cndMoney.amount), cndType = \(cndMoney.type)")
var ausMoney : (amount: Double, type:String) = (1*1.36, "aud")
print("usValue = \(usdMoney.amount), usType = \(usdMoney.type) is audValue = \(ausMoney.amount), audType = \(ausMoney.type)")
